...
- **Runtime** – `app::runtime` glues together state preparation, managed image acquisition, host checks, and VM/Vizier lifecycle. It relies on `CliResult`/`CliError` and prints warnings plus progress from inside helper functions.
...
- **Process lifecycle** – Legacy `app::runtime::start_broker`/`stop_broker` helpers have been removed. The library runtime now focuses on VM processes and treats any leftover broker pidfiles as stale artefacts.
...
Guests: No running VMs detected.

VM      STATE      CPU/MEM  UPTIME  FORWARDS
devbox  stopped  2/2048MiB       —  2222->22/tcp, 8080->80/tcp

Legend: STATE reflects pidfile inspection; FORWARDS list configured host → guest ports.
`castra status --json` mirrors these fields and reports whether any VMs are running.
...
### Removed `castra broker`

The hidden broker command has been removed. Harness-driven Vizier streams replace the legacy TCP broker.

## Configuration & Filesystem Expectations

- `ProjectConfig` fields: `file_path`, semantic `version`, human `project_name`, `vms` (vector of `VmDefinition`), `state_root` (defaults under `~/.castra/projects/<slug>-<hash>`), `workflows`, and accumulated parse `warnings`.
...
- Managed images live under `<state_root>/images`; host logs live under `<state_root>/logs`.
- PID files: `<state_root>/<vm>.pid` for each VM. Legacy `<state_root>/broker.pid` files are removed on sight.
...
## VM & Vizier Lifecycle (Current State)
...
- Existing tests focus on parsing (`config.rs`) and CLI argument parsing (`cli.rs`). No tests exercise the high-level workflows end-to-end or assert CLI text output, which will be a gap once the library API is introduced.
