...
pub fn prepare_runtime_context(project: &ProjectConfig) -> CliResult<RuntimeContext> {
    ...
    fs::create_dir_all(&image_storage_root).map_err(|err| CliError::PreflightFailed {
        message: format!(
...
pub fn check_host_capacity(project: &ProjectConfig, force: bool) -> CliResult<()> {
    ...
    if let Some(total_memory) = system.total_memory().checked_mul(1024) {
        ...
        warnings.push(
            "Unable to determine host memory capacity; skipping memory safety check.".to_string(),
        );
    }
...
pub fn ensure_ports_available(project: &ProjectConfig) -> CliResult<()> {
...
pub fn start_broker(project: &ProjectConfig, context: &RuntimeContext) -> CliResult<()> {
...
pub fn ensure_vm_assets(
    vm: &VmDefinition,
    context: &RuntimeContext,
) -> CliResult<AssetPreparation> {
    match &vm.base_image {
...
pub fn shutdown_vm(vm: &VmDefinition, state_root: &Path) -> CliResult<bool> {
...
pub fn shutdown_broker(state_root: &Path) -> CliResult<bool> {
...
fn build_boot_overrides(
...
) -> CliResult<Option<BootOverrides>> {
    let kernel_path = match spec.qemu.kernel {
    ...
    let initrd_path = match spec.qemu.initrd {
        Some(kind) => Some(resolve_managed_artifact(kind, paths).ok_or_else(|| {
            CliError::PreflightFailed {
                message: format!(
    ...
    let kernel = kernel_path.ok_or_else(|| CliError::PreflightFailed {
        message: format!(
